...
int main()
{
   int n;
    while(true)
    {
        ...
        sf1(temp);
        if(temp<=0)break;
        string binary = std::bitset<32>(temp).to_string(); //to binary
        string a="";
        string b="";
        bool ans=false;
        for (int i = binary.length()-1; i >=0 ; --i)
        {
            if(ans==false && binary[i]=='1')
            {
                a+='1';
                b+='0';
                ans=true;
            }
            else if(ans==true && binary[i]=='1')
            {
                a+='0';
                b+='1';
                ans=false;
        ...
        std::bitset<32> bitss(b);
        std::cout << bits.to_ulong() <<" "<<bitss.to_ulong() << std::endl;
